          ...
          <TabsContent value="board" className="mt-0 outline-none">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {STATUS_COLUMNS.map((column) => (
                <div
                  key={column}
                  className={`flex flex-col gap-4 p-3 rounded-2xl transition-all duration-300 min-h-[700px] border-2 ${dragOverColumn === column
                ...
                >
                  <div className="flex items-center justify-between px-1">
                    <h3 className="font-semibold text-sm uppercase tracking-wider text-muted-foreground/70 flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${column === 'Done' ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.4)]' :
                        ...
                        {tasks.filter(st => st.parent_id === bt.id).map(st => (
                          <tr key={st.id} className="hover:bg-primary/5 transition-colors cursor-pointer" onClick={() => {
                            ...
                            <td className="px-6 py-3">
                              <span className={`px-2 py-0.5 rounded-full text-[9px] font-black ${st.status === 'Done' ? 'bg-green-500/10 text-green-500' : 'bg-primary/10 text-primary'
                                }`}>{st.status}</span>
                            </td>
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4 flex-1">
                            <button
                              ...
                              className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${st.status === 'Done' ? 'bg-green-500 border-green-500 text-white scale-110 shadow-lg shadow-green-500/20' : 'border-muted-foreground/20 hover:border-primary/50'
                                }`}
                            >
                              ...
                              <DropdownMenuContent align="end" className="w-48">
                                ...
                                {STATUS_COLUMNS.map(s => (
                                  <DropdownMenuItem
                                    ...
                                    className="gap-2"
                                  >
                                    <div className={`w-2 h-2 rounded-full ${s === 'Done' ? 'bg-green-500' : s === 'In Progress' ? 'bg-blue-500' : s === 'Break' ? 'bg-orange-500' : s === 'Blocked' ? 'bg-destructive' : 'bg-muted-foreground/30'
                                      }`} />
                                    {s}
                                  </DropdownMenuItem>
                                ))}
...
function TaskItem({ task, subtasks, onDragStart, onDragEnd, onDragOver, onDrop, onStatusChange, onDelete, onClick, onOpenDetails, isDragging, onMove }: any) {
  ...
  return (
    <Card
      className={`group hover:border-primary/50 transition-all duration-300 cursor-grab active:cursor-grabbing border-border shadow-md overflow-hidden ${isDragging ? 'opacity-40 scale-[0.98] border-primary border-dashed' : 'hover:shadow-xl hover:shadow-primary/5'
      ...
      <CardHeader className="p-5 space-y-3">
        <div className="flex justify-between items-start">
          <div className="flex items-center gap-2.5">
            ...
            <span className={`text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-wider ${task.priority === 'High' || task.priority === 'Urgent' ? 'bg-destructive/10 text-destructive border border-destructive/20' :
